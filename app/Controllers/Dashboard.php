...
class Dashboard extends BaseController
{
    ...
    public function index()
    {
        ...
        log_message('debug', 'Dashboard: Detected user role: ' . ($userRole ?? 'null'));
        ...
        // For admin users, proceed to admin dashboard
        // No additional check needed since routes already enforce admin access

        // Get dashboard analytics based on user role - simplified for debugging
        $shopName = $this->session->get('user_shop_name');
        ...
        try {
            ...
            $analytics = ['orders' => 0, 'revenue' => '&#8369;0.00', 'new_users' => 0, 'active_sessions' => 0];
            $userStats = ['by_role' => ['admin' => 0, 'customer' => 0], 'recent_registrations' => 0];
            $systemMetrics = ['uptime' => 'N/A', 'database_size' => 'N/A', 'total_tables' => 0, 'memory_usage' => 'N/A'];
            $monthlyAnalytics = ['revenue' => '&#8369;0.00'];
            $totalProducts = 0;
        ...
        try {
            $salesChartData = $this->dashboardModel->getSalesChartData();
            $topProducts = $this->dashboardModel->getTopProducts(5);
            $lowStockAlerts = $this->dashboardModel->getLowStockAlerts(10);
            $revenueOverview = $this->dashboardModel->getRevenueOverview();
            $monthlySalesTrend = $this->dashboardModel->getMonthlySalesTrend();
        } catch (\Exception $e) {
            log_message('error', 'Dashboard new features error: ' . $e->getMessage());
            $salesChartData = [];
            $topProducts = [];
            $lowStockAlerts = [];
            $revenueOverview = [
    ...
    {
        // Prevent admin users from accessing customer areas
        $userRole = $this->session->get('user_role');
        if ($userRole === 'admin') {
            return redirect()->to('/dashboard')
                ->with('error', 'Admins cannot access customer areas. Use admin dashboard.');
        }
    ...
    {
        // Check if user is admin and redirect to admin products
        $userRole = $this->session->get('user_role');
        if ($userRole === 'admin') {
            return redirect()->to('/products')->with('info', 'Redirected to admin products management.');
        }
    ...
    public function productDetails($id)
    {
        ...
        // Get reviews data
        $userId = (int) $this->session->get('user_id');
        $reviews = $this->reviewModel->getProductReviews($id, 'approved', 10);
        $averageRatings = $this->reviewModel->getProductAverageRatings($id);
        $ratingDistribution = $this->reviewModel->getProductRatingDistribution($id);
        $userReview = $this->reviewModel->getUserProductReview($userId, $id);
        $canReview = $this->reviewModel->canUserReviewProduct($userId, $id);
    ...
    {
        // Redirect to the review form page with order data
        return redirect()->to('/customer/review/form?order_id=' . $order['id'])
                       ->with('order', $order);
    }
    ...
    public function submitReview()
    {
        try {
            ...
            if ($rating < 1 || $rating > 5) {
                return redirect()->back()->with('error', 'Please select a valid overall rating.');
            }

            // Simple review data - skip complex validation for now
            $reviewData = [
                'product_id' => $productId,
                'order_id' => $orderId,
                'user_id' => $userId,
                'rating' => $rating,
                'flavor_rating' => (int) $this->request->getPost('flavor_rating') ?: null,
                'hit_strength_rating' => (int) $this->request->getPost('hit_strength_rating') ?: null,
                'review_title' => $this->request->getPost('review_title') ?: null,
                'review_text' => $this->request->getPost('review_text') ?: null,
            ...
            if ($this->reviewModel->save($reviewData)) {
                ...
                return redirect()->to('/customer/orders')->with('success', $message);
            } else {
                $errors = $this->reviewModel->errors();
                return redirect()->back()->with('error', 'Failed to submit review. ' . implode(', ', $errors));
            }
        } catch (\Exception $e) {
    ...
    public function markReviewHelpful()
    {
        ...
        if ($this->reviewModel->markHelpful($reviewId)) {
            return $this->response->setJSON([
                'success' => true,
    ...
    /**
     * Admin: Get reviews for a product
     */
    public function getProductReviews($productId)
    {
        ...
        $reviews = $this->reviewModel->getReviewsForAdmin(
    ...
    public function approveReview($reviewId)
    {
        ...
        $adminNotes = $this->request->getPost('admin_notes') ?: null;
        
        if ($this->reviewModel->approveReview($reviewId, $adminNotes)) {
            return $this->response->setJSON([
                'success' => true,
    ...
    public function rejectReview($reviewId)
    {
        ...
        if ($this->reviewModel->rejectReview($reviewId, $adminNotes)) {
            return $this->response->setJSON([
                'success' => true,
    ...
    /**
     * Admin: Get reviews for a specific order
     */
    public function getOrderReviews($orderId)
    {
        ...
        // Get reviews for this specific order
        ...
        // Otherwise, render the reviews page
        return view('admin/order_reviews', [
            'reviews' => $reviews,
            'orderId' => $orderId
        ]);
    }
    ...
    /**
     * Customer: Get reviews for a specific order
     */
    public function getCustomerOrderReviews($orderId)
    {
        $authCheck = $this->checkAuth();
        if ($authCheck !== true) {
            return $authCheck;
        ...
        // Get reviews for this specific order (only approved reviews for customers)
        $reviews = $this->reviewModel->getReviewsByOrder($orderId, 'approved');
        ...
        // Otherwise, render the reviews page
        return view('customer/order_reviews', [
            'reviews' => $reviews,
            'orderId' => $orderId,
            'order' => $order
        ]);
    ...
    public function reviewForm()
    {
        ...
        if (!$orderId) {
            return redirect()->to('/customer/orders')->with('error', 'Order ID is required.');
        }
        ...
        if ((int) ($order['created_by'] ?? 0) !== (int) $this->session->get('user_id')) {
            return redirect()->to('/customer/orders')->with('error', 'You can only review your own orders.');
        }

        // Check if the order is eligible for review (completed or cancelled)
        if (!in_array($order['delivery_status'], ['completed', 'cancelled'])) {
            return redirect()->to('/customer/orders')->with('error', 'You can only review completed or cancelled orders.');
        }

        return view('customer/review_form', [
            'order' => $order,
            'page_title' => 'Write Review - Order ' . $order['reference_number']
        ]);
    }
}
