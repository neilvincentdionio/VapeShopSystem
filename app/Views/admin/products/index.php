...
<body>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="reviews-summary">
                                            ...
                                            $reviewStats = $reviewModel->getProductAverageRatings($product['id']);
                                            $pendingCount = $reviewModel->getProductReviewCount($product['id'], 'pending');
                                            ?>
                                            <?php if ($reviewStats['total_reviews'] > 0): ?>
                                                <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
                                                    <i class="fas fa-star" style="color: #ffc107; font-size: 0.9rem;"></i>
                                                    <span style="font-weight: 600;"><?= $reviewStats['avg_rating'] ?></span>
                                                    <span style="color: #666; font-size: 0.85rem;">(<?= $reviewStats['total_reviews'] ?>)</span>
                                                </div>
                                                <?php if ($pendingCount > 0): ?>
                                                    <div style="margin-top: 0.25rem;">
                                                        <span style="background: #fff3cd; color: #856404; padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.75rem; font-weight: 600;">
                                                            <?= $pendingCount ?> pending
                                                        </span>
                                                    </div>
                                                <?php endif; ?>
                                            <?php else: ?>
                                                <span style="color: #999; font-size: 0.85rem;">No reviews</span>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            ...
                                            <button onclick="manageReviews(<?= $product['id'] ?>, '<?= esc($product['name']) ?>')" 
    ...
    function manageReviews(id, name) {
        // Open modal to manage reviews for this product
        const modal = document.createElement('div');
        modal.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center;';
        modal.innerHTML = `
            <div style="background:white;border-radius:12px;padding:2rem;max-width:900px;max-height:80vh;overflow:auto;position:relative;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem;">
                    <h2 style="margin:0;font-size:1.5rem;">Reviews for "${name}"</h2>
                    <button onclick="this.closest('[style*=position:fixed]').remove()" style="background:none;border:none;font-size:1.5rem;cursor:pointer;">&times;</button>
                </div>
        ...
        // Load reviews
        fetch('<?= site_url('admin/reviews/product/') ?>' + id)
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    displayReviews(data.reviews);
                } else {
                    document.getElementById('reviewsContent').innerHTML = '<p style="text-align:center;color:#999;">Failed to load reviews</p>';
                }
            })
            .catch(() => {
                document.getElementById('reviewsContent').innerHTML = '<p style="text-align:center;color:#999;">Network error</p>';
            });
    }
    ...
    function displayReviews(reviews) {
        ...
        if (reviews.length === 0) {
            html = '<p style="text-align:center;color:#999;padding:2rem;">No reviews found for this product.</p>';
        } else {
                ...
                html += `
                    <div style="border:1px solid #e0e0e0;border-radius:8px;padding:1rem;margin-bottom:1rem;">
                        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:0.5rem;">
                            <div>
                                <div style="font-weight:600;color:#333;">${review.user_name}</div>
                                <div style="color:#666;font-size:0.9rem;">${new Date(review.created_at).toLocaleDateString()} - Order #${review.reference_number}</div>
                            </div>
                            <span style="background:${statusBg};color:${statusColor};padding:0.25rem 0.75rem;border-radius:12px;font-size:0.8rem;font-weight:600;">
                                ${review.status}
                            </span>
                        </div>
                        
                        <div style="display:flex;align-items:center;margin-bottom:0.5rem;">
                            ${Array.from({length:5}, (_, i) => `<i class="fas fa-star" style="color:${i < review.rating ? '#ffc107' : '#e0e0e0'};font-size:0.9rem;"></i>`).join('')}
                            <span style="margin-left:0.5rem;color:#666;font-size:0.9rem;">${review.rating}/5</span>
                        </div>
                        
                        ${review.flavor_rating ? `
                            <div style="display:flex;align-items:center;margin-bottom:0.25rem;">
                                <i class="fas fa-palette" style="color:#00bcd4;margin-right:0.5rem;font-size:0.85rem;"></i>
                                <span style="color:#00bcd4;font-size:0.85rem;">Flavor: ${review.flavor_rating}/5</span>
                            </div>
                        ` : ''}
                        
                        ${review.hit_strength_rating ? `
                            <div style="display:flex;align-items:center;margin-bottom:0.5rem;">
                                <i class="fas fa-fire" style="color:#ff6b6b;margin-right:0.5rem;font-size:0.85rem;"></i>
                                <span style="color:#ff6b6b;font-size:0.85rem;">Hit Strength: ${review.hit_strength_rating}/5</span>
                            </div>
                        ` : ''}
                        
                        ${review.review_title ? `<h4 style="margin:0.5rem 0;font-size:1rem;color:#333;">${review.review_title}</h4>` : ''}
                        ${review.review_text ? `<p style="color:#666;line-height:1.5;margin:0.5rem 0;">${review.review_text}</p>` : ''}
                        
                        ${review.status === 'pending' ? `
                            <div style="display:flex;gap:0.5rem;margin-top:1rem;">
                                <button onclick="approveReview(${review.id})" style="background:#27c56f;color:white;border:none;padding:0.5rem 1rem;border-radius:6px;cursor:pointer;font-size:0.85rem;">
                                    <i class="fas fa-check"></i> Approve
                                </button>
                                <button onclick="rejectReview(${review.id})" style="background:#dc3545;color:white;border:none;padding:0.5rem 1rem;border-radius:6px;cursor:pointer;font-size:0.85rem;">
